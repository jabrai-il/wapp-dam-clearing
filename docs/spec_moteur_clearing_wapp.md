...
# Addendum v0.5 (23 septembre 2026) : architecture

12. **Solveur abstrait.** Le moteur n'appelle plus SciPy ou HiGHS directement : `Clearing` et `PriceDeterminer` reçoivent un objet conforme au protocole `Solver` (trois problèmes canoniques : MILP du clearing, LP générique avec duals, projection quadratique sur un polyèdre). `HighsSolver` est l'implémentation par défaut ; un solveur commercial s'ajoute sans toucher à l'algorithme, ce qui permet de comparer les temps de calcul et de vérifier que les résultats ne dépendent pas de l'outil. Aucun changement de résultat (rejeux OMIE et GME identiques).
