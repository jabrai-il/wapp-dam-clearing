...
\newcommand{\mc}[1]{\begin{tabular}[b]{@{}r@{}}#1\end{tabular}}
\newcommand{\mcl}[1]{\begin{tabular}[c]{@{}l@{}}#1\end{tabular}}
\usepackage{multirow}
\usepackage{colortbl}
...
\date{Working paper, version 0.6 (brouillon), septembre 2026}
...
\smallskip
\noindent Nous formalisons le clearing prescrit par le Code comme un programme linéaire mixte de maximisation du bien-être régional et caractérisons les prix zonaux compatibles avec une allocation par les conditions de complémentarité des ordres et des flux. Trois propriétés en découlent : l'existence de prix cohérents à blocs figés, leur non-existence en général pour les blocs (d'où la règle du rejet paradoxal, seule compatible avec le Code) et la terminaison de la procédure de fixation des blocs incohérents. Une implémentation ouverte (Python, solveur libre) accompagne l'article, validée sur 20 cas unitaires et par le rejeu de six journées réelles : marché ibérique (OMIE, courbes agrégées, prix retrouvé au centime sur 250 des 290 quarts d'heure) et marché italien (GME, carnet d'ordres complet, 21 zones, prix zonal exact dans 91~\% des cas, transits reproduits à quelques MW près).
...
\medskip
\noindent We formalise the clearing prescribed by the Code as a mixed-integer linear programme maximising regional welfare and characterise the zonal prices compatible with an allocation through the complementarity conditions of hourly orders and flows. Three properties follow: coherent prices exist once blocks are fixed; they do not exist in general for blocks (hence the paradoxical-rejection rule, the only one compatible with the Code); and the block-fixing procedure terminates. An open implementation (Python, free solver) accompanies the paper, validated on 20 unit cases and by replaying real days: three days of the Iberian market (OMIE, aggregated curves, official price recovered to the cent on 250 of 290 quarter-hours) and three days of the Italian market (GME, complete order book, 21 zones, zonal price recovered to the cent in 91~\% of cases and internal flows reproduced within a few MW).
...
\item \textbf{Ordres horaires.} Un ordre $o\in O_{z,h}$ porte une quantité $q_o\ge 0$, un prix limite $p_o$ et un sens $s_o\in\{+1,-1\}$ (achat, vente). L'implémentation admet aussi un ordre portant sur un ensemble $H_o$ de MTU consécutifs avec un ratio unique (un ordre de 60~min sur un marché au quart d'heure, EPD-2025 §5.1) ; il entre dans l'équilibre de chaque MTU de $H_o$ et sa cohérence s'apprécie sur la moyenne arithmétique des prix de $H_o$. Le Code, au pas horaire, n'en a pas besoin ; le rejeu du marché italien (section~\ref{sec:gme}) en a eu besoin.
...
\item \textbf{Prix} : blocs figés, résolution du LP et lecture des duaux comme point de référence $\hat\pi$ ; puis programme linéaire en $\pi$ sur $\Pi(x^\star,f^\star)$ minimisant $M\sum_{b\text{ accepté}}V_b\,\sigma_b + \|\pi-\hat\pi\|_1$, où $\sigma_b\ge0$ est la violation de cohérence du bloc $b$. En option (\texttt{price\_rule = midpoint}), le point de référence est remplacé par le milieu $m_{z,h}$ de l'intervalle de prix admissible de chaque $(z,h)$, donné par les ordres horaires acceptés et rejetés, et les prix sont ensuite affinés au sens des moindres carrés dans $\Pi(x^\star,f^\star)$ : $\min\sum_{z,h}(\pi_{z,h}-m_{z,h})^2$. C'est la règle de l'enchère européenne (EPD-2025, annexe C), et la seule qui reproduise ses prix publiés (section~\ref{sec:gme}).
...
Choisir les prix dans $\Pi$ en minimisant l'incohérence rend le résultat indépendant de ce hasard numérique. Pour la reproductibilité des résultats, c'est une exigence, pas un raffinement. Reste, à l'intérieur de $\Pi$, le choix du point : le Code ne dit rien, l'enchère européenne prend le milieu de l'intervalle, et un SMO qui retiendrait une autre règle (borne basse, borne haute, dual du solveur) publierait des prix différents pour la même allocation. C'est le quatrième paramètre de la section~\ref{sec:ouverts}.

Le code (\url{https://github.com/jabrai-il/wapp-dam-clearing}, licence MIT, version 0.4.0 archivée sur Zenodo sous le DOI concept \href{https://doi.org/10.5281/zenodo.22902401}{10.5281/zenodo.22902401}) tient en huit modules Python (objets, validation, modèle, solveurs, prix, clearing, entrées-sorties, ligne de commande) et n'a pour dépendances que NumPy, SciPy et l'interface Python de HiGHS. Le solveur est isolé derrière un protocole à trois méthodes (MILP, LP avec duals, projection quadratique), de sorte qu'un solveur commercial peut être substitué sans modifier l'algorithme. L'instance de la section~\ref{sec:application} (4 zones, 526 ordres horaires, 42 blocs dont 8 familles liées et 7 groupes exclusifs, 6 liaisons orientées, 24 heures) se résout en moins de 0.1~s sur un ordinateur portable ; le passage à quinze zones et quelques milliers d'ordres reste linéaire en pratique tant que le nombre de blocs se compte en centaines, la difficulté combinatoire venant d'eux seuls.

\section{Validation}\label{sec:validation}

\subsection{Cas unitaires}

Chaque règle de la section~\ref{sec:formulation} fait l'objet d'un cas unitaire dont la valeur attendue est calculée à la main ; les tableaux~\ref{tab:validation} et~\ref{tab:validation2} les résument (20 tests). Tous sont exécutés à chaque modification du code par l'intégration continue du dépôt.
...
\toprule
 & \multicolumn{3}{c}{N1 : toutes les marches offertes} & \multicolumn{3}{c}{N2 : décisions complexes données} & N2, écarts \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
Journée & \mc{prix\\exacts} & \mc{écart\\moyen} & \mc{écart\\max} & \mc{prix\\exacts} & \mc{écart\\moyen} & \mc{écart\\max} & \mc{dans\\l'intervalle} \\
\midrule
20 janv. 2026 & 0 / 96 & 12.75 & 42.38 & 86 / 96 & 0.03 & 0.59 & 10 / 10 \\ \rowsep
10 juin 2026 & 17 / 96 & 10.12 & 38.34 & 94 / 96 & 0.01 & 0.78 & 2 / 2 \\ \rowsep
15 sept. 2026 & 0 / 98 & 14.40 & 50.67 & 70 / 98 & 0.20 & 4.00 & 28 / 28 \\
\bottomrule
\end{tabular}
\caption{Rejeu de trois journées OMIE, écarts en EUR/MWh. « Prix exacts » : couples (MTU, zone) où le prix du moteur égale le prix officiel à 0.01~EUR/MWh près ; la journée du 15 septembre compte deux MTU en market splitting, d'où 98 couples. « Écarts dans l'intervalle » : écarts N2 restants dont le prix officiel est strictement compris entre la dernière vente casée et le premier achat casé.}
\label{tab:omie}
\end{table}
...
Ce rejeu vérifie la formation des prix par ordres horaires et échanges preneurs de prix, à l'échelle réelle (4\,000 à 6\,000 marches par MTU, 20~s par journée). Il ne vérifie ni l'allocation des capacités entre zones ni les blocs, dont OMIE ne publie pas le détail. Le marché italien le permet.

\subsection{Cas de référence multi-zones : rejeu du marché italien}\label{sec:gme}

GME, l'opérateur italien, publie sept jours après chaque journée le carnet d'ordres complet et anonymisé de son marché du jour pour le lendemain, avec pour chaque ordre la zone, la période, le prix, la quantité, le volume attribué et le statut, et pour chaque bloc son profil et son ratio minimal d'acceptation ; il publie à côté les limites de transit entre zones par quart d'heure, les transits réalisés et les prix zonaux \citep{gme2026}. Le marché compte sept zones géographiques reliées par des capacités limitées, des zones virtuelles pour les frontières à enchères explicites (Suisse, Monténégro, Malte, Corse) et des zones de couplage sans ordres (France, Autriche, Slovénie, Grèce), soit 21 zones et 42 arcs orientés. Le script \texttt{examples/gme/replay\_gme.py} reconstruit la journée : les ordres au statut accepté, rejeté ou paradoxalement rejeté sont soumis tels quels, la position nette de chaque zone de couplage est lue dans les transits et injectée comme ordre preneur de prix, et l'allocation des capacités entre zones italiennes est laissée au moteur.

Deux règles de l'enchère européenne, sans objet au pas horaire du Code, ont dû être ajoutées au moteur pour reproduire les prix.

\begin{itemize}
\item \textbf{Ordres multi-MTU.} Le marché italien est au quart d'heure mais admet des ordres de 60 et 30~min, qui portent un ratio unique sur leurs MTU et sont dans la monnaie sur la moyenne des prix (EPD-2025 §5.1). Traités comme quatre marches indépendantes, ils font apparaître des ordres de vente rejetés « dans la monnaie » sur un quart d'heure isolé.
\item \textbf{Règle de prix dans l'intervalle admissible.} Avec la règle par défaut du moteur (dual du solveur), l'écart moyen au prix officiel reste de 0.48~EUR/MWh ; avec le milieu de l'intervalle au sens des moindres carrés (EPD-2025, annexe~C), il tombe à 0.03.
\end{itemize}
...
\begin{tabular}{lrrrrrrr}
\toprule
 & & & \multicolumn{3}{c}{N2 : prix zonaux} & \multicolumn{2}{c}{N2 : transits internes} \\
\cmidrule(lr){4-6}\cmidrule(lr){7-8}
Journée & ordres & blocs & \mc{prix\\exacts} & \mc{écart\\moyen} & \mc{écart\\max} & \mc{écart\\moyen (MW)} & \mc{arcs saturés\\reproduits} \\
\midrule
20 janv. 2026 & 289\,248 & 79 & 628 / 672 & 0.06 & 5.40 & 27.1 & 160 / 209 \\ \rowsep
10 juin 2026 & 300\,110 & 128 & 619 / 672 & 0.03 & 0.65 & 5.9 & 110 / 120 \\ \rowsep
15 sept. 2026 & 291\,551 & 173 & 589 / 672 & 0.15 & 15.25 & 1.3 & 128 / 131 \\
\bottomrule
\end{tabular}
\caption{Rejeu de trois journées du marché italien, blocs fixés à leurs volumes acceptés (N2). « Ordres » : marches soumises au clearing après expansion des ordres 30 et 60~min ; « exacts » : couples (quart d'heure, zone italienne) où le prix du moteur égale le prix officiel à 0.01~EUR/MWh près ; « arcs saturés reproduits » : couples (quart d'heure, arc interne) saturés dans la publication et saturés par le moteur à 1~MW près.}
\label{tab:gme}
\end{table}

Sur les trois journées, le moteur retrouve le prix zonal officiel au centime dans 91~\% des cas (1\,836 sur 2\,016) et reproduit les transits internes, congestions comprises, à quelques MW près (tableau~\ref{tab:gme}). Les écarts restants se concentrent sur une poignée de quarts d'heure où le moteur trouve, dans notre lecture des règles, une allocation de bien-être légèrement supérieur à celle publiée ; EUPHEMIA résout un programme quadratique mixte sous limite de temps, avec une recherche locale sur les blocs qui s'interrompt quand le temps restant devient insuffisant, et ne garantit pas l'optimum \citep[§7.4, §7.10]{euphemia2025}. Le rejeu prend 10 à 20~s par journée.

\textbf{Blocs libres (N3).} Le clearing de la journée entière (96 MTU, 66\,000 à 72\,000 marches après agrégation par prix, 76 à 168 blocs libres) prend 9 à 11~s et trois itérations de cohérence par journée ; le tableau~\ref{tab:gme_n3} croise, pour chaque bloc, le statut publié par GME et celui donné par le moteur.
...
\begin{tabular}{lrrrrrrr}
\toprule
 & & \multicolumn{4}{c}{\textbf{Même décision} GME / moteur} & & \\
\cmidrule(lr){3-6}
Journée & \mc{blocs\\(soumis)} & accepté & rejeté & \mc{parad. rejeté\\(MILP)} & \mc{parad. rejeté\\(cohérence)} & \mc{Décision\\différente} & \mc{Prix\\exacts} \\
\midrule
20 janv. 2026 & 76 (79) & 51 & 20 & 3 & 2 & 0 & 628 / 672 \\ \rowsep
10 juin 2026 & 121 (128) & 66 & 48 & 4 & 2 & 1 & 518 / 672 \\ \rowsep
15 sept. 2026 & 168 (173) & 49 & 108 & 9 & 2 & 0 & 583 / 672 \\
\midrule
Total & 365 (380) & 166 & 176 & 16 & 6 & 1 & 1\,729 / 2\,016 \\
\bottomrule
\end{tabular}
\caption{Rejeu de trois journées avec blocs libres (N3). « Blocs (soumis) » : blocs entrés dans le MILP, et entre parenthèses blocs soumis ; la différence est faite de blocs à profil non contigu, admis par GME mais exclus par le Code (\MC{13.1.2.1}~b) et rejetés à la validation. Les quatre colonnes centrales comptent les blocs où le moteur retrouve la décision publiée ; « parad. rejeté » est le statut PREJ de GME (bloc dans la monnaie aux prix publiés mais rejeté), que le moteur rend soit par le MILP, soit par l'itération de cohérence de la section~\ref{sec:algo}. La colonne « rejeté / rejeté » inclut, le 20 janvier et le 10 juin, un bloc rejeté par GME que le moteur classe paradoxalement rejeté (même décision, autre étiquette). Le seul écart, le 10 juin, est un bloc de vente à ratio minimal 0.7 accepté à 96~\% par EUPHEMIA et paradoxalement rejeté par le moteur ; Prix exacts : couples (quart d'heure, zone italienne) au centime près, journée entière.}
\label{tab:gme_n3}
\end{table}

Trois lectures du tableau~\ref{tab:gme_n3}.

\begin{itemize}
\item \textbf{Accepter et rejeter.} Sur 365 blocs libres, le moteur prend la même décision que l'enchère européenne pour 364 : les blocs acceptés, rejetés et paradoxalement rejetés se retrouvent à l'identique, deux journées sur trois sans aucun écart.
\item \textbf{Le rejet paradoxal est reproduit.} Les 22 blocs que GME publie comme paradoxalement rejetés le sont aussi par le moteur, 16 par le MILP lui-même et 6 par l'itération de cohérence, qui est donc bien le mécanisme que le Code appelle sans le décrire.
\item \textbf{Le seul écart tient à l'heuristique.} Le bloc du 10 juin est à 2.8~EUR/MWh de la monnaie : c'est le type de solution qu'une recherche locale sous limite de temps trouve ou manque \citep[§7.4, §7.10]{euphemia2025}. Il déplace le prix de toutes les zones couplées de 2 à 10~EUR/MWh sur ses 24 quarts d'heure, ce qui explique l'essentiel des 154 écarts de prix de cette journée ; les deux autres journées retrouvent en N3 les prix de N2.
\end{itemize}

\textbf{Ce que N3 apporte.} N2 prend les décisions de blocs comme données et ne peut donc rien en dire ; il valide la partie linéaire du moteur (croisement, allocation des capacités, règle de prix). N3 valide la partie combinatoire, celle des propositions~\ref{prop:blocs} et~\ref{prop:terminaison} : ratio minimal, rejet paradoxal, itération de cohérence, confrontés à 365 décisions réelles au lieu de nos seuls cas unitaires. Il permet surtout une mesure que N2 ne peut pas produire : à ordres identiques, le surplus de l'allocation publiée comparé à celui de l'allocation recalculée (tableau~\ref{tab:gme_welfare}). Sur les trois journées, à blocs fixés (N2) le surplus recalculé égale le surplus publié à quelques dizaines d'euros près, et à blocs libres (N3) le moteur trouve un surplus supérieur ou égal à celui d'EUPHEMIA, de 38 à 2\,573~EUR sur 2.4 à 2.7 milliards, soit au plus un pour un million : la solution publiée est admissible et à distance négligeable de l'optimum du programme \eqref{eq:obj}--\eqref{eq:cons}, ce qui établit que cette formulation, avec les règles de blocs de la section~\ref{sec:formulation}, rend compte des résultats de l'enchère européenne à la précision de son heuristique. Le script \texttt{examples/gme/welfare\_gme.py} produit ce tableau.
...
\toprule
 & & \multicolumn{2}{c}{N2 : blocs fixés} & \multicolumn{3}{c}{N3 : blocs libres} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-7}
Journée & \mc{Surplus publié\\(GME, EUR)} & \mc{surplus\\(EUR)} & \mc{écart\\(EUR)} & \mc{surplus\\(EUR)} & \mc{écart\\(EUR)} & \mc{blocs décidés\\autrement} \\
\midrule
20 janv. 2026 & 2\,717\,628\,663 & 2\,717\,628\,648 & $-$15 & 2\,717\,629\,712 & +1\,049 & 0 \\ \rowsep
10 juin 2026 & 2\,389\,313\,090 & 2\,389\,313\,060 & $-$31 & 2\,389\,315\,664 & +2\,573 & 1 \\ \rowsep
15 sept. 2026 & 2\,431\,986\,004 & 2\,431\,986\,042 & +38 & 2\,431\,986\,042 & +38 & 0 \\
\bottomrule
\end{tabular}
\caption{Surplus des ordres (somme signée des prix limites fois les volumes acceptés) de l'allocation publiée par GME, de l'allocation N2 (blocs fixés aux volumes publiés, ordres simples recalculés heure par heure) et de l'allocation N3 (blocs libres, journée entière), calculés sur les mêmes ordres, hors positions de couplage. Les écarts sont pris par rapport au surplus publié, positifs quand le moteur fait mieux ; ils valent au plus un pour un million. À blocs identiques, N2 retrouve le surplus publié à quelques dizaines d'euros près ; N3 le dépasse le 10 juin par le seul bloc décidé autrement, et les deux autres jours par de petits déplacements de volumes entre ordres à la monnaie.}
\label{tab:gme_welfare}
\end{table}
...
\item \textbf{Prix dans l'intervalle admissible} : quand plusieurs prix sont compatibles avec l'allocation, milieu de l'intervalle au sens des moindres carrés (règle européenne, seule à reproduire les prix publiés, section~\ref{sec:gme}), dual du solveur, ou borne ; le Code ne dit rien.
...
\item \textbf{Publication} : format des courbes agrégées et des résultats, sans lequel aucune reproduction indépendante n'est possible.
\end{enumerate}
...
Ces trois points, avec les dix paramètres de la section~\ref{sec:ouverts}, devraient figurer dans la DAM Clearing Procedure avant le lancement, et être vérifiables par un second moteur.

Les limites de cette version sont connues : fixation gloutonne des blocs sans branchement, dont la perte de bien-être est mesurée et peut dépasser l'effet étudié (variante~D), partage du délestage limité à l'intérieur d'une zone, pas de marché OTC ni de droits de transport, pas de règlement financier ; les cas de référence européens vérifient le couplage multi-zones (Italie) mais les blocs seulement sur les décisions publiées d'une journée, pas sur l'ensemble des règles de famille, que les données italiennes ne mettent pas en jeu.

Les étapes suivantes sont, dans l'ordre :

\begin{enumerate}
\item le rejeu des premiers jeux d'essai ou résultats publiés par le SMO, sur le modèle des rejeux ibérique et italien des sections~\ref{sec:omie} et~\ref{sec:gme} ;
